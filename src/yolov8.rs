...
pub fn init_model(path: &str, app_ctx: &mut RknnAppContext) -> Result<()> {
    ...
    app_ctx.input_attrs = input_attrs.clone();
    app_ctx.output_attrs = output_attrs;

    if input_attrs[0].fmt == _rknn_tensor_format_RKNN_TENSOR_NCHW {
        info!("model is NCHW input fmt");
        app_ctx.model_channel = input_attrs[0].dims[1] as i32;
        app_ctx.model_height = input_attrs[0].dims[2] as i32;
        app_ctx.model_width = input_attrs[0].dims[3] as i32;
    } else {
        info!("model is NHWC input fmt");
        app_ctx.model_height = input_attrs[0].dims[1] as i32;
        app_ctx.model_width = input_attrs[0].dims[2] as i32;
        app_ctx.model_channel = input_attrs[0].dims[3] as i32;
    }
    info!(
